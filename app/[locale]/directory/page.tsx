...
export default async function DirectoryPage({
  ...
  // Registered exporters (DB) + sample data; falls back to samples if the DB is down.
  const exporters = await listDirectoryExporters();
  ...
  return (
    <div className="container-page py-12">
      ...
      <div className="mt-8">
        <SyriaMap locale={locale as Locale} dict={dict} exporters={exporters} />
      </div>
      <div className="mt-8">
        <DirectoryBrowser locale={locale as Locale} dict={dict} exporters={exporters} initialSector={initialSector} />
      </div>
    </div>
  );
}
